...
export default function StudyPage() {
  ...
  // Function to load all study cards from local storage or Supabase
  const handleViewAllCards = async () => {
    setLoading(true);
    let items = await db.items.toArray();

    // If local storage has no cards and user is online, pull all non-deleted study items
        ...
        {loading ? (
          ...
          <div className="text-center">
            ...
            <div className="flex gap-4 justify-center">
              <button
                onClick={handleViewAllCards}
                className="px-4 py-2 bg-emerald-600 hover:bg-emerald-500 rounded text-sm font-semibold transition text-white"
              >
                View All Cards
              </button>
              <Link
